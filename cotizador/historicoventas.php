...
while (($row = mssql_fetch_array($sp))) 
    if ($row["Id_Mod"] == 1 && $row["ID_Opc"] == 1 && $row['CodUsr'] == $UsrId) {
        $OkOpc = true;
        break;
    }
...
<td  valign="top" class="label_left_right_top_bottom" STYLE="PADDING-LEFT:20px; PADDING-RIGHT:20px; TEXT-ALIGN:left">
	<table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="1" ALIGN="center">
	<tr><td>
                <h2>Ordenes de Compras Autorizadas</h2>
		...
		<tr>
			<td class="titulo_tabla" width="10%" align="middle">Fecha</td>
                        <td class="titulo_tabla" width="10%" align="middle">Cotizaci&oacute;n</td>
                        <td class="titulo_tabla" width="10%" align="middle">N&uacute;mero</td>
			<td class="titulo_tabla" width="15%" align="middle">Rut</td>
			<td class="titulo_tabla" width="45%" align="middle" style="text-align: left">Nombre</td>
                        <td class="titulo_tabla" width="10%" align="left" style="text-align: center">Pago</td>
		</tr>
		<?php
			$j = 0;
			$iTotPrd = 0;
			$result = mssql_query("vm_s_cot_hisvta", $db);
			while ($row = mssql_fetch_array($result)) {
				...
				echo "   <td class=\"".$clase2."\" style=\"TEXT-ALIGN: center; \"><a href=\"javascript:ver_preview(".$row['Cod_Cot'].")\">".$row['Cod_Odc']."</a></td>\n";
				...
				if ($row["Cod_TipPer"] == 1)
					...
					echo "   <td class=\"".$clase2."\" style=\"TEXT-ALIGN: left\">".$row['RznSoc_Per']."</td>\n";
                                if ($row['Arc_Adj'] != " ") {
                                    ...
                                    echo "<a href=\"javascript:ver_comprobante('".$row['Arc_Adj']."')\">SI</a>";
                                    echo "</td>\n";
                                }
                                else {
                                    echo "   <td class=\"".$clase2."\" style=\"TEXT-ALIGN: center \">";
		...
		<?php if ($iTotPrd == 0) { ?>
		<tr>
			<td colspan="6" style="PADDING-TOP: 10px; TEXT-ALIGN: CENTER; PADDING-BOTTOM: 10px" class="label_left_right_bottom">
