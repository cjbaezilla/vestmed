...
function PutParametro ($tag, $value) {
    return " ".$tag."=\"".$value."\"";
}
...
function cuerpo_cotizacion ($id_pais,$folio,$link) {
  ...
  $aTipSvc   = array(-1 => "", 0 => "(a Domicilio)", 1 => "(a Sucursal del Carrier)");
  ...
  if (($row = mssql_fetch_array($result1))) {
	...
	if ($despacho == 1) {
		...
		$nom_suc = $row["Nom_Suc"];
		$dir_suc = $row["Dir_Suc"];
		$tip_svc = $row['Cod_TipSvcCrr'];
		if ($tip_svc == null) $tip_svc = -1;
	}
	$num_per =$row["Num_Per"];
	$cod_pre = $row["Cod_Pre"];
        //$observacion = utf8_decode($row["obs_cot"]);
        $observacion = $row["obs_cot"];
        if ($observacion == " ") $observacion = "&nbsp;";
        $rubro = "Cotizaci&oacute;n Nro ".$num_cot;
	$pesoPer = $row['Cot_Peso'];
	$estaturaPer = $row['Cot_Estatura'];
	$flgTer = $row['Cot_FlgTer'];
  ...
  $cuerpo = "<html xmlns=\"http://www.w3.org/1999/xhtml\"><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\"/></head><body>";
  ...
  $cuerpo.="<TR>\n";
  $cuerpo.="<TD style=\"WIDTH: 220px; TEXT-ALIGN: right; ".$label3."\" valign=top>\n";
  $cuerpo.="<STRONG>Peso</STRONG> :&nbsp;</TD>\n";
  $cuerpo.="<TD style=\"WIDTH: 280px; TEXT-ALIGN: left; PADDING-LEFT: 5px; ".$dato3."\" valign=center>\n";
  $cuerpo.=$pesoPer." kg</TD>\n";
  $cuerpo.="</TR>\n";
  $cuerpo.="<TR>\n";
  $cuerpo.="<TD style=\"TEXT-ALIGN: right; ".$label33."\"><STRONG>Estatura</STRONG> :&nbsp;</TD>\n";
  $cuerpo.="<TD style=\"TEXT-ALIGN: left; PADDING-LEFT: 5px; ".$dato33."\" valign=center>\n";
  $cuerpo.=$estaturaPer.($flgTer == 1 ? " cm (Producto para Terceros)" : " cm")."</TD>\n";
  $cuerpo.="</TR>\n";
  ...
  $cuerpo.=($despacho == 1 ? "SI" : "NO")." ".$aTipSvc[$tip_svc];
  $cuerpo.="</TD>\n";
  $cuerpo.="</TR>\n";
...
function formar_topmail ($id_pais,$titulo) {
  ...
  $cuerpo.="<title>Aviso Vestmed</title>\n";
  $cuerpo.="</HEAD>\n";
...
function display_login($Cod_Per, $Cod_Clt, $link, $Cod_Cot) {
	...
	$result = mssql_query ("vm_per_s ".$Cod_Per, $link) or die ("No se pudo leer datos del usuario (".$Cod_Per.")");
	...
	$linea ="<form ID=\"F1\" AUTOCOMPLETE=\"off\" method=\"POST\" name=\"F1\">\n";
        ...
        $linea.="<li class=\"back-verde\"><a href=\"vermismensajes.php\">Mensajes</a></li>\n";
	...
	if ($cod_clt_usr != $Cod_Clt) {
		$result = mssql_query ("vm_cli_s ".$Cod_Clt, $link) or die ("No se pudo leer datos del cliente");
                $nombre_clt="";
		if ($row = mssql_fetch_array($result)) 
			$nombre_clt	= utf8_encode($row["RznSoc_Per"]);
		mssql_free_result($result); 
...
function solicitar_login() {
        ...
        $linea.= "<li class=\"back-verde inputp\" style=\"float:left;\"><input type=\"password\" id=\"dfclave\" name=\"dfclave\" tabIndex=\"2\" /></li>\n";
        $linea.= "<li class=\"back-verde\" style=\"float:left;\"><a href=\"javascript:ValidarLogin()\">ENTRAR</a></li>\n";
        //$linea.= "<li class=\"back-verde\" style=\"float:left;\"><a href=\"index2.htm\">INTRANET</a></li>\n";
...
function eliminarLastLevelSession() {
	$buffer = "";
	$buffersession = split(";", $_SESSION['buffer']);
	for ($i=0; $i<(count($buffersession)); $i++) $buffer.=$buffersession[$i].";";
	$_SESSION['buffer'] = substr($buffer,0,strlen($buffer)-1);
}

function DisplayLevelSession($class) {
	$session = split(";", $_SESSION[($class == "") ? 'buffer' : 'buffer2']);
...
function DisplayLastSession($titulo, $pagina) {
	$session = split(";", $_SESSION['buffer']);
	$i = 0;
	foreach ($session as $key => $value) {
		if ($i == (count($session)-1)) {
			$link = split(",", $value);
			echo "<a href=\"".$link[0].($i == 0 ? "?" : "&")."page=".$pagina."\">".$titulo."</a>";	
			break;
		}
		$i++;
	}
...
function DisplayLevelSession2($class) {
