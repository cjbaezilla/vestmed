...
$filter = (isset($_GET['filter'])) ? intval(ok($_GET['filter'])) : 2;
...
<tr><td>
	...
	<?php
		...
		while ($row = mssql_fetch_array($result)) {
			...
			if ($Est_Odr == "2")
				echo "<a href=\"grilla.php?prd=".$row['Cod_Prd']."&fec=$Fec_Odr&cod=$Num_Odr&filter=$filter".($row['Ctd_Prd'] == 0 ? "&tipo=1" : "")."\">Edit</a> / <a href=\"javascript:DelPrd('".$row['Cod_Prd']."')\">Delete</a></TD>\n";
			else if ($Est_Odr == "3") {
	...
	<?php
		...
		while ($row = mssql_fetch_array($result)) {
			...
			if ($Est_Odr == "2")
				echo "<a href=\"grilla.php?prd=".$row['Cod_Prd']."&fec=$Fec_Odr&cod=$Num_Odr&filter=$filter\">Edit</a> / <a href=\"javascript:DelPrd('".$row['Cod_Prd']."')\">Delete</a></TD>\n";
			else if ($Est_Odr == "3") {
		...
		<legend>
          ...
          <table cellpadding="2" cellspacing="0" width="100%px" height="40px">
            ...
            <tr>
			  ...
			  <form ID="F3" AUTOCOMPLETE="off" method="POST" name="F3" ACTION="reponer_ins.php?filter=<?php echo $filter; ?>" onsubmit="return checkDataFichaOdr(this,2)">
              <td style="width: 75px;">
