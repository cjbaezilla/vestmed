<?php
	...
	case 40:
		...
		if ($row = mssql_fetch_array($result)) {
			...
			mssql_close ($db);
			header("Location: compra.php?cod=$Num_Odr&fec=$Fec_Mov&filter=$caso"); 	
		}
		break;
			...
			if ($key == "seleccionadof") {
				...
				foreach ($value as $index => $value2) {
					...
					if ($row = mssql_fetch_array($result)) {
						...
						if ($caso == 1) {
							$query = "vm_odr_i '$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd,'$Flg_Rep','A',$Cod_Nvt,$Num_Lin";
		...
		if ($row = mssql_fetch_array($result)) {
			...
			if ($Ctd_Prd  != "" && $Cod_Sze != "") {
				...
				if ($row = mssql_fetch_array($result)) {
					$Cod_Prd = $row['Cod_Prd'];
					if ($caso == 1) {
					   $query = "vm_odr_i '$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd,'$flag','U'";
						//echo $query;
						$result = mssql_query($query, $db);
						mssql_close ($db);
						...
						$result = mssql_query("vm_odr_i_rep $Num_Odr,'$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',0, $Ctd_Prd", $db);
						mssql_close ($db);
						header("Location: compra.php?cod=$Num_Odr&fec=$Fec_Mov"); 				
					}
					else if ($caso == 20) {
						//echo "vm_odr_i_rep $Num_Odr,'$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd";
						$result = mssql_query("vm_odr_i_rep $Num_Odr,'$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd, 0", $db);
						mssql_close ($db);
						header("Location: compra.php?cod=$Num_Odr&fec=$Fec_Mov&filter=20"); 				
					}
					else if ($caso == 3) {
						//echo "vm_odr_i_rep $Num_Odr,'$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd";
