...
<script language="JavaScript">
	...
	function CrearOdc() {
		f6.action = "reponer_ins.php?filter=40";
		f6.submit();
	}
...
<tr><td style="PADDING-TOP: 20px">
		...
		while ($row = mssql_fetch_array($result)) {
			if (($Perfil == 1 && $row['Ctd_Prd'] > 0) || $Perfil == 2) {
				...
				case 1:
					...
					break;
				case 2:
					$link = "compra.php?cod=".$row['Num_Odr']."&fec=$Fec_Mov&filter=$filter";
					break;
				}
	...
	<?php if ($iTotPrd == 0) { 
	      $numcol = ($Est_Odr == '3') ? 5 : 4;
	?>
	<TR>
		<TD colspan="<?php echo $numcol ?>" style="PADDING-TOP: 10px; TEXT-ALIGN: CENTER; PADDING-BOTTOM: 10px" class="label_left_right_bottom">
		NO EXISTEN PRODUCTOS PENDIENTES DE ENV&Iacute;O. SI DESEA AGREGAR PINCHE <A HREF="reponer_ins.php?filter=<?php echo ($Perfil == 1) ? "5" : "4"; ?>">AQU&Iacute;</A>
		</TD>
	</TR>
	<TR>
		<form ID="searchList" AUTOCOMPLETE="off" method="POST" name="searchList" action="reponer_ins.php?filter=<?php echo ($Perfil == 1) ? "5" : "4"; ?>">
		<TD colspan="<?php echo $numcol ?>" style="PADDING-TOP: 10px; TEXT-ALIGN: RIGHT; PADDING-BOTTOM: 10px">
		<input type="submit" name="CreateOdr" value="Crear Orden de Reposici&oacute;n" tabindex="20" class="button2" style="width:220px;" />&nbsp;
		<input type="button" name="CreateOdc" value="Crear Orden de Compra" onclick="javascript:CrearOdc()" tabindex="21" class="button2" style="width:220px;" />
