<?php
	...
	foreach ($_POST as $key => $value) {
		...
		else if (($caso == 1 || $caso == 3) && left($key,6) == "TxtSze") {
			if ($value != "") {
	...
	if ($caso == 1) {
		...
		$pageretorno = "compra.php?cod=$Num_Odr&fec=$Fec_Mov";
	}
	else if ($caso == 3) {
		$name_sp = "vm_odc_i";
	...
	if ($row = mssql_fetch_array($result)) {
		...
		if (isset($aListaTallas))
			foreach ($aListaTallas as $Cod_Sze => $Ctd_Prd) {
				if ($caso == 1) 
					$result = mssql_query("vm_odr_i"." '$Fec_Mov','$Cod_Dsg','$Cod_Pat','$Cod_Sze',$Ctd_Prd,'$flag','U',NULL,NULL,$Num_Odr", $db);
				else if ($caso == 2) 
