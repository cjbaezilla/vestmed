...
if (!isset($_SESSION['usuario'])) {
    if (!isset($_POST["usuario"])) header("Location: ../index1.php");
...
$titulo = "Hist&oacute;rico";

$estados = array("1" => "Pendiente","2" => "Cerrada","3" => "Procesada");
?>
...
<P align="left"><strong><?php echo $titulo ?></strong></P>
<P align="center">
<TABLE WIDTH="90%" BORDER="0" CELLSPACING="0" CELLPADDING="1" ALIGN="center">
<form ID="F2" AUTOCOMPLETE="off" method="POST" name="F2" ACTION="historicorep.php">
<tr><td class="dato">
	<fieldset>
...
<tr><td style="PADDING-TOP: 20px">
	...
	<form ID="F4" AUTOCOMPLETE="off" method="POST" name="F4" action="ocompra_ins.php">
	<?php
		$j = 0;
		$Mca = "";
		$iTotPrd = 0;
		...
		$xis = 0;
		if ($Perfil == 3)
			$result = mssql_query("vm_odchis_s $tipcna, '$fechain', '$fechafi'", $db);
		else
			$result = mssql_query("vm_odrhis_s $tipcna, '$fechain', '$fechafi'", $db);
		...
		while ($row = mssql_fetch_array($result)) {
			...
			if ($Perfil == 3) {
				...
				echo "   <TD class=\"".$clase2."\" style=\"TEXT-ALIGN: center\">".$estados[$row['Est_Odc']]."</TD>\n";
			} else {
				$afecha  = split("/", $row['Fec_Odc']);
				$Fec_Mov = $afecha[2].$afecha[1].$afecha[0];
			...
			switch ($Perfil) {
			case 1:
				$link = "reponer.php?cod=".$row['Num_Odr']."&fec=".$row['Fec_Odr']."&acc=historico";
				break;
			case 2:
				$link = "compra.php?cod=".$row['Num_Odr']."&fec=".$row['Fec_Odr']."&acc=historico";
				break;
			case 3:
				$link = "detcompra.php?cod=".$row['Num_Odc']."&fec=".$row['Fec_Odc']."&acc=historico";
				break;
			}
